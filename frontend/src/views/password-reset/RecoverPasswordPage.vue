  ...
  methods: {
    async sendRecoveryEmail() {
      ...
      try {
        ...
        if (data.success) {
          this.successMessage = data.message || 'Si el email existe, recibirás instrucciones.';
          this.email = ''; // Limpiar campo
        } else {
          this.errorMessage = data.message || 'Error al solicitar la recuperación (respuesta no exitosa).';
        }
      } catch (error) {
        console.error("Error en componente recover password:", error);
        this.errorMessage = error.response?.data?.message || error.message || 'Error al enviar el enlace de recuperación.';
      }
    }
  }
};
