  ...
  methods: {
    async updatePassword() {
      ...
      try {
        ...
        if (data.success) {
          this.successMessage = data.message || 'Contraseña actualizada correctamente.';
          ...
          this.errorMessage = data.message || 'Error al actualizar la contraseña (respuesta no exitosa).';
        }
      } catch (error) {
        console.error("Error en componente reset password:", error);
        this.errorMessage = error.response?.data?.message || error.message || 'Error al actualizar la contraseña o token inválido/expirado.';
      }
    }
  }
};
