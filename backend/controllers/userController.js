...
exports.getAllUsers = async (req, res) => {
  try {
    const users = await User.findAll({
      ...
      include: [
        { model: Role, as: "userRole", attributes: ["id", "role_name"] },
      ],
    });
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al obtener los usuarios." });
  }
};
...
exports.getTeachers = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al obtener los profesores." });
  }
};
...
exports.searchUsers = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al buscar los usuarios." });
  }
};
...
exports.getStudents = async (req, res) => {
  try {
    ...
    const studentsWithRole = students.map((user) => ({
      id: user.id,
      username: user.username,
      name: user.name,
      surname: user.surname,
      email: user.email,
      rol: user.rol,
      role_name: user.userRole ? user.userRole.role_name : "Alumno",
      active: user.active,
      profile_image: user.profile_image,
    }));

    res.json({ success: true, students: studentsWithRole });
  } catch (error) {
    console.error("Error en getStudents:", error);
    res
      .status(500)
      .json({ success: false, message: "Error al obtener los alumnos." });
  }
};

// Obtener la lista de asignaturas (ID y nombre) que imparte un profesor específico
exports.getProfessorSubjectList = async (req, res) => {
  try {
    ...
    const subjects = await Subject.findAll({
      ...
      include: [
        {
          model: Students_teachers_relation,
    ...
    res
      ...
      .json({
        success: false,
        message: "Error al obtener la lista de asignaturas del profesor.",
      });
  }
};
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    if (req.user.id !== parseInt(id) && req.user.role !== 2) {
      // Asumiendo que 2 es administrador
      return res
        .status(403)
        .json({
          success: false,
...
exports.uploadProfilePicture = async (req, res) => {
  try {
    ...
    if (error instanceof multer.MulterError) {
      ...
      error.message &&
      error.message.includes("Solo se permiten archivos de imagen")
    ) {
