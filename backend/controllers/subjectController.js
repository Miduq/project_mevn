...
exports.createSubject = async (req, res) => {
  ...
  if (!subject || typeof subject !== 'string' || subject.trim() === '') {
    return res.status(400).json({
      success: false,
      message: 'El nombre de la asignatura es obligatorio y no puede estar vacío.',
    });
  }
...
exports.getSubjectById = async (req, res) => {
  try {
    const { id } = req.params;
    if (isNaN(id)) {
      return res.status(400).json({ success: false, message: 'ID de asignatura inválida.' });
    }
    ...
    res.status(500).json({ success: false, message: 'Error internos del servidor.' });
  }
};
...
exports.deleteSubject = async (req, res) => {
  try {
    ...
    if (existingRelation) {
      return res.status(409).json({
        success: false,
        message: 'No se puede eliminar la asignatura porque está siendo utilizada por un estudiante o profesor.',
      });
    }
